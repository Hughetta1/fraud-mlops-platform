...
# Loads real Kaggle credit card fraud data when available; falls back to
# sklearn.make_classification synthetic data otherwise.
# Single interface: create_fraud_dataset(n_samples=None) → DataFrame.
# Stratified sampling guarantees at least 1 fraud case for any sample size.
...
def create_fraud_dataset(n_samples: Optional[int] = None) -> pd.DataFrame:
    """
    Unified data entry point for the fraud detection pipeline.

    Loads the real Kaggle Credit Card Fraud dataset if available on disk
    (data/raw/creditcard.csv). Falls back to synthetic data generation if
    the file is not found.

    Args:
        n_samples: If provided, returns a stratified random sample of the
                   specified size. If None, returns the full real dataset
                   (or 10,000 synthetic samples as fallback).

    Returns:
        DataFrame with at minimum: Time, V1-VN features, Amount, Class.
    """
    from .data_loader import is_real_data_available, load_real_data, validate_dataset
    ...
    if is_real_data_available():
        ...
        if n_samples is not None and n_samples < len(df):
            ...
            n_fraud = min(n_fraud, total_fraud)
            n_normal = n_samples - n_fraud

            df_fraud = df[df["Class"] == 1].sample(n=n_fraud, random_state=42)
            df_normal = df[df["Class"] == 0].sample(n=n_normal, random_state=42)
            df = pd.concat([df_fraud, df_normal], ignore_index=True)
            df = df.sample(frac=1, random_state=42).reset_index(drop=True)

        print(
            f"Loaded real data: {len(df):,} transactions, "
            f"fraud rate: {df['Class'].mean():.4%}"
        )
        return df

    # Fallback: synthetic data
    n = n_samples if n_samples else 10000
    config = DatasetConfig(n_samples=n, random_state=42)
    generator = FraudDatasetGenerator(config)
    df = generator.create_fraud_dataset()
    print(f"Generated synthetic data: {len(df):,} transactions")
    return df
