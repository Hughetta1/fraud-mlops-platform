...
class FeatureParams:
    """
    Offline parameters computed during fit_transform() and reused during
    transform() for single-row / small-batch inference.

    These bridge the gap between batch training and online inference:
    parameters are learned from the full training set and applied to
    individual transactions without requiring batch-level statistics.
    """
    # Amount feature params
    amount_mean: Optional[float] = None
    amount_std: Optional[float] = None
    amount_median: Optional[float] = None
    amount_boxcox_lambda: Optional[float] = None
    amount_bin_quantile_edges: Optional[Any] = None  # Index for pd.cut
    amount_bin_log_edges: Optional[Any] = None

    # Statistical feature params (PCA fitted on V columns)
    v_pca: Optional[Any] = None  # Fitted PCA object

    def is_empty(self) -> bool:
        return self.amount_mean is None
...
class AdvancedFeatureEngineering:
    ...
    def __init__(self, config: Optional[FeatureConfig] = None, target_column: str = "Class"):
        ...
        # Offline→online parameter bridge
    ...
    def create_amount_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Amount" in df.columns:
            # Backward compat: models saved before FeatureParams was added
            if not hasattr(self, "params"):
                self.params = FeatureParams()
            is_online = self.is_fitted  # True during transform(), False during fit_transform()

            # ---- Online features: computable per-row without batch stats ----
            ...
            # ---- Offline features: use stored params during transform ----
            # Amount mean/std/median: fit on batch, apply to single rows
            if not is_online:
                amount_mean = float(df["Amount"].mean())
                amount_std = float(df["Amount"].std())
                amount_median = float(df["Amount"].median())
                self.params.amount_mean = amount_mean
                self.params.amount_std = amount_std
                self.params.amount_median = amount_median
            else:
                amount_mean = self.params.amount_mean or 0.0
                amount_std = self.params.amount_std or 1.0
                amount_median = self.params.amount_median or 0.0
            ...
            if not is_online:
                ...
                # Apply using stored median (MAD denominator approximated)
                mad_denom = np.abs(df["Amount"] - amount_median).median()
                if mad_denom < 1e-8:
                    mad_denom = 1.0
                df["amount_mad_score"] = np.abs(df["Amount"] - amount_median) / mad_denom
            ...
            # Box-Cox: fit lambda on training set, apply lambda during transform
            if (df["Amount"] > 0).all():
                if not is_online:
                    try:
                        amount_vals = df["Amount"].values + 1
                        if amount_vals.std() > 1e-8:
                            _, lam = stats.boxcox(amount_vals)
                            self.params.amount_boxcox_lambda = float(lam)
                        else:
                            self.params.amount_boxcox_lambda = 0.0
                    except Exception:
                        self.params.amount_boxcox_lambda = 0.0

                lam = self.params.amount_boxcox_lambda
                if lam is not None:
                    from scipy.special import boxcox as apply_boxcox
                    df["amount_boxcox"] = apply_boxcox(df["Amount"].values + 1, lam)
                else:
                    df["amount_boxcox"] = np.log(df["Amount"].values + 1)

            # Binning: fit edges on training set, apply pd.cut during transform
            if not is_online:
                try:
                    _, edges_q = pd.qcut(df["Amount"], q=10, labels=False, duplicates="drop", retbins=True)
                    self.params.amount_bin_quantile_edges = edges_q
                    df["amount_bin_quantile"] = pd.cut(df["Amount"], bins=edges_q, labels=False, include_lowest=True)
                except Exception:
                    df["amount_bin_quantile"] = 0
                    self.params.amount_bin_quantile_edges = None

                try:
                    _, edges_log = pd.cut(np.log1p(df["Amount"]), bins=10, labels=False, retbins=True)
                    self.params.amount_bin_log_edges = edges_log
                    df["amount_bin_log"] = pd.cut(np.log1p(df["Amount"]), bins=edges_log, labels=False, include_lowest=True)
                except Exception:
                    df["amount_bin_log"] = 0
                    self.params.amount_bin_log_edges = None
            else:
                edges_q = self.params.amount_bin_quantile_edges
                if edges_q is not None:
                    df["amount_bin_quantile"] = pd.cut(df["Amount"], bins=edges_q, labels=False, include_lowest=True)
                else:
                    df["amount_bin_quantile"] = 0

                edges_log = self.params.amount_bin_log_edges
                if edges_log is not None:
                    df["amount_bin_log"] = pd.cut(np.log1p(df["Amount"]), bins=edges_log, labels=False, include_lowest=True)
                else:
                    df["amount_bin_log"] = 0
    ...
    def create_velocity_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Time" in df.columns and "Amount" in df.columns:
            # During online inference (single row): velocity features need
            # historical data via Redis. Zero-fill as safe default.
            # Phase 3 P2 (online_features.py) will replace this with real values.
            if self.is_fitted and len(df) < 10:
                for window in [10, 30, 100]:
                    df[f"velocity_{window}"] = 0.0
                    df[f"tx_count_{window}"] = 1
                    df[f"amount_mean_{window}"] = df["Amount"]
                    df[f"amount_std_{window}"] = 0.0
                df["time_since_last"] = 0.0
                df["time_since_last_fillna"] = 3600.0
                df["velocity_change"] = 0.0
                df["is_velocity_increasing"] = 0
                df["tx_frequency_hour"] = 0.0
            else:
                # Batch mode: full rolling-window computation
                ...
                df_sorted["is_velocity_increasing"] = (df_sorted["velocity_change"] > 0).astype(int)
    ...
    def create_statistical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if v_columns:
            ...
            # Fit PCA on training data; store and apply during transform.
            if len(v_columns) > 3:
                if not hasattr(self, "params"):
                    self.params = FeatureParams()
                is_online = self.is_fitted
                try:
                    if not is_online or self.params.v_pca is None:
                        pca = PCA(n_components=3, random_state=42)
                        pca_features = pca.fit_transform(v_data)
                        if not is_online:
                            self.params.v_pca = pca
                    else:
                        pca = self.params.v_pca
                        pca_features = pca.transform(v_data)
    ...
    def create_anomaly_features(self, df: pd.DataFrame, fit: bool = True) -> pd.DataFrame:
        ...
        if len(numerical_cols) > 0:
            if fit:
                ...
                self.anomaly_feature_cols_ = list(numerical_cols)
                ...
                # Align columns to what was seen during fit
                fit_cols = getattr(self, "anomaly_feature_cols_", list(numerical_cols))
                X = df.reindex(columns=fit_cols, fill_value=0).fillna(0)

                iso_forest = self.anomaly_detectors["isolation_forest"]
                df["anomaly_score_iforest"] = iso_forest.predict(X)
                df["anomaly_score_iforest_raw"] = iso_forest.score_samples(X)
            else:
                X = df[list(numerical_cols)].fillna(0)
    ...
    def create_clustering_features(self, df: pd.DataFrame, fit: bool = True) -> pd.DataFrame:
        ...
        if len(feature_cols) > 0:
            if fit:
                ...
                self.cluster_feature_cols_ = list(feature_cols)
                ...
                # Align columns to what was seen during fit
                fit_cols = getattr(self, "cluster_feature_cols_", feature_cols)
                X = df.reindex(columns=fit_cols, fill_value=0).fillna(0)
    ...
    def _mahalanobis_distance(self, X: pd.DataFrame) -> np.ndarray:
        ...
        Handles edge cases: single-row input (degenerate covariance),
        constant features, or SVD non-convergence.
        """
        if len(X) < 2:
    ...
    def fit_transform(self, df: pd.DataFrame, target_column: Optional[str] = None) -> pd.DataFrame:
        ...
        # Reset fitted state so steps re-compute offline params
        self.is_fitted = False
        self.params = FeatureParams()
        ...
        pipeline_steps = [
            ...
            ("[Categorical encoding]", lambda x: self.encode_categorical_features(x, fit=True)),
        ...
        # Snapshot feature columns before selection/reduction for transform-time alignment
        self.fitted_feature_cols_ = [c for c in df.columns if c != self.target_column]
        ...
        if target is not None and self.config.n_features_to_select > 0:
            ...
            print(f"  [OK]Shape after selection: {df.shape}")
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ---- Column alignment: ensure transform columns match fit columns ----
        # Some features (boxcox, PCA sub-features etc.) are conditionally created.
        # Align to the feature set seen during fit to prevent downstream model errors.
        if hasattr(self, "fitted_feature_cols_"):
            extra_cols = [c for c in df.columns if c not in self.fitted_feature_cols_ and c != self.target_column]
            missing_cols = [c for c in self.fitted_feature_cols_ if c not in df.columns and c != self.target_column]
            if extra_cols:
            ...
            ordered = [c for c in self.fitted_feature_cols_ if c in df.columns]
            if self.target_column in df.columns:
        ...
        # Select same features first (match fit_transform order)
        if self.selected_features:
            ...
            # Preserve target column before filtering
            target_series = df[self.target_column] if self.target_column in df.columns else None

            df = df[self.selected_features]

            if target_series is not None and self.target_column not in self.selected_features:
                df[self.target_column] = target_series

        # Apply dimensionality reduction (after feature selection, matching fit order)
        if self.config.enable_pca:
...
def validate_pipeline_performance():
    ...
    print(f"[OK]Original features: {df_train.shape[1]}")
    print(f"[OK]Features after engineering: {df_train_transformed.shape[1]}")
    print(f"[OK]Selected features: {len(report['selected_features'])}")
    print(f"[OK]Fit time: {fit_time:.2f} seconds")
    print(f"[OK]Transform time: {transform_time:.2f} seconds")
    print(f"[OK]PCA variance explained: {report.get('pca_explained_variance', 0):.2%}")
